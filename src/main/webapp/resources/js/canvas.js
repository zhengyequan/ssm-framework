...
var oDots = [], canvas = null, ctx = null, oDotA = null, oDotB = null, W = null, H = null;
...
var colorToRGB = function(color, alpha) {
	...
	var r = color >> 16 & 255, g = color >> 8 & 255, b = color >> 4 & 255, a = (alpha < 0) ? 0
			: ((alpha > 1) ? 1 : alpha);
	if (a === 1) {
		return "rgb(" + r + "," + g + "," + b + ")"
	} else {
...
var makeDots = function() {
	for (var i = 0; i < options.numDot; i++) {
		if (options.isRangeRad) {
			options.radDot = Math.floor(ran(2,
					options.radDot))
		}
		var oDot = new Practicle(options.dotColor,
				options.radDot, options.opacity);
		oDot.x = ran(0, W);
		oDot.y = ran(0, H);
...
var rotate = function(x, y, sin, cos, reverse) {
	return {
		x : (reverse) ? (x * cos + y * sin)
				: (x * cos - y * sin),
		y : (reverse) ? (y * cos - x * sin)
				: (y * cos + x * sin)
	}
};
var checkCollision = function(oDot0, oDot1) {
	var dx = oDot1.x - oDot0.x, dy = oDot1.y - oDot0.y, dist = Math
			.sqrt(dx * dx + dy * dy);
	if (dist < oDot0.radius + oDot1.radius) {
		var angle = Math.atan2(dy, dx), sin = Math
				.sin(angle), cos = Math.cos(angle);
		var pos0 = {
			x : 0,
			y : 0
		};
		var pos1 = rotate(dx, dy, sin, cos, true);
		var vel0 = rotate(oDot0.vx, oDot0.vy, sin, cos,
				true);
		var vel1 = rotate(oDot1.vx, oDot1.vy, sin, cos,
				true);
		var vxTotal = vel0.x - vel1.x;
		vel0.x = ((oDot0.mass - oDot1.mass) * vel0.x + 2
				* oDot1.mass * vel1.x)
				/ (oDot0.mass + oDot1.mass);
		vel1.x = vxTotal + vel0.x;
		var absV = Math.abs(vel0.x) + Math.abs(vel1.x), overlap = (oDot0.radius + oDot1.radius)
				- Math.abs(pos0.x - pos1.x);
		pos0.x += vel0.x / absV * overlap;
		pos1.x += vel1.x / absV * overlap;
		var pos0F = rotate(pos0.x, pos0.y, sin, cos,
				false);
		var pos1F = rotate(pos1.x, pos1.y, sin, cos,
				false);
		oDot1.x = oDot0.x + pos1F.x;
		oDot1.y = oDot0.y + pos1F.y;
		oDot0.x = oDot0.x + pos0F.x;
		oDot0.y = oDot0.y + pos0F.y;
		var vel0F = rotate(vel0.x, vel0.y, sin, cos,
				false);
		var vel1F = rotate(vel1.x, vel1.y, sin, cos,
				false);
		oDot0.vx = vel0F.x;
		oDot0.vy = vel0F.y;
		oDot1.vx = vel1F.x;
...
var drawLine = function(oDot0, oDot1) {
	...
	if (dist < options.lineDist) {
		ctx.save();
		ctx.strokeStyle = colorToRGB(options.lineColor,
				(options.opacity - 0.4) <= 0 ? 0.1
						: options.opacity - 0.4);
		ctx.beginPath();
...
var execute = function() {
	...
	for (var i = 0; i < options.numDot - 1; i++) {
		...
		for (var j = i + 1; j < options.numDot; j++) {
			oDotB = oDots[j];
			options.isBallCollisionTest
					&& checkCollision(oDotA, oDotB);
